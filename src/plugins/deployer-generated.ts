 ...
 * @lastGenerated 2025-08-29T14:35:58.412Z
 */
...
export class DecisionDeployer {
  ...
  constructor(options: DecisionDeployerOptions = {}) {
    ...
    if (typeof options.systemExposure === "string") {
      this.systemExposure =
        (Object.values(SystemExposureLevel).find(
          (v) => v === options.systemExposure,
        ) as SystemExposureLevel) || undefined;
    } else {
    ...
    if (typeof options.humanImpact === "string") {
      this.humanImpact =
        (Object.values(HumanImpactLevel).find(
          (v) => v === options.humanImpact,
        ) as HumanImpactLevel) || undefined;
    } else {
    ...
    if (
      ...
      this.systemExposure !== undefined &&
      this.utility !== undefined &&
      this.humanImpact !== undefined
    ) {
  ...
  toVector(): string {
    ...
    const exploitationVector =
      ...
      this.exploitation ||
      "";
    const system_exposureVector =
      { small: "S", controlled: "C", open: "O" }[
        this.systemExposure?.toString?.()?.toUpperCase?.() ?? ""
      ] ||
      this.systemExposure ||
      "";
    const utilityVector =
      ...
      this.utility ||
      "";
    const human_impactVector =
      { low: "L", medium: "M", high: "H", very_high: "V" }[
        this.humanImpact?.toString?.()?.toUpperCase?.() ?? ""
      ] ||
      this.humanImpact ||
      "";
  ...
  static fromVector(vectorString: string): DecisionDeployer {
    ...
    if (!match) {
      throw new Error(
        `Invalid vector string format for Deployer: ${vectorString}`,
      );
    }
    ...
    return new DecisionDeployer({
      exploitation:
        ...
        exploitationMatch,
      systemExposure:
        { S: "small", C: "controlled", O: "open" }[
          system_exposureMatch || ""
        ] || system_exposureMatch,
      utility:
      ...
      humanImpact:
        { L: "low", M: "medium", H: "high", V: "very_high" }[
          human_impactMatch || ""
        ] || human_impactMatch,
    });
  }
